  ...
  if (
    ...
    if (typeof koe_knowking_msg === 'undefined') {
      ...
      koe_knowking_msg.setAttribute(
        'style',
        'position:fixed;top:0;left:0;padding:5px 10px;height:30px;line-height:30px;text-align:center;background-color:#000;color:#FFF;font-size:26px;z-index:99999999'
      );
    ...
    koe_knowking_xhr.onreadystatechange = function () {
      if (
        ...
        var audio_url =
          'http://tts.baidu.com/text2audio?lan=zh&ie=UTF-8&spd=5&text=没有人比我更懂，懂了个寂寞';
        // 如果返回结果正确
        if (json.status === 1) {
          // 更改百度接口参数内容为词义
          audio_url =
            'http://tts.baidu.com/text2audio?lan=zh&ie=UTF-8&spd=5&text=没有人比我更懂' +
            koe_knowking_selected_text +
            '，' +
            json.content;
          // 创建音频
          var audio = document.createElement('audio');
          audio.setAttribute('id', 'koe_knowking_audio');
